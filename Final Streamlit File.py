...
option = st.sidebar.selectbox('Please choose one of the following pages to display', ['Home Page', 'Word Cloud', 'Machine Learning','Frequencies' ])
...
if option == 'Home Page':
    ...
    with col1:
        ...
        st.write("-  check out this [Deepnote Code](https://deepnote.com/workspace/drewquag-2e9b6724-412b-4149-af9a-d088e32ec141/project/Streamlit-and-Deepnote-Duplicate-d0e6130a-dc18-4f69-be95-903728811574/%2Fnotebook.ipynb)")
...
#Make sure to fix error when region is np.nan
    ...
    st.write('_This page will allow you to view the best wines varieties made in each country (max 10 wine varieties). This is done by averaging the points for each wine variety in each respective country and then ranking them.')
    country = st.sidebar.selectbox('Please choose a country', df['country'].dropna().sort_values().unique())
    st.pyplot(find_wine_variety_frequency(country))
    st.write('_Here is a list of the top wineries (max 10) in your chosen country ranked by average points received by every wine they produce.')
